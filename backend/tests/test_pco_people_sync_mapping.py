...
from app.services.pco_people_sync import fetch_active_people


def _person(person_id: str, name: str, email_ids: list[str], phone_ids: list[str]) -> dict:
    return {
        "type": "Person",
        "id": person_id,
...
def test_fetch_active_people_picks_primary_email_and_first_phone():
...
def test_fetch_active_people_falls_back_when_no_email_flagged_primary():
...
def test_fetch_active_people_handles_no_contact_info_at_all():
...
def test_fetch_active_people_requests_only_active_status():
    with patch(
    ...
    assert kwargs["params"]["where[status]"] == "active"
