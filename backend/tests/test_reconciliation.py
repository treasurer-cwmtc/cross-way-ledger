...
def test_import_run_dedups_on_reimport():
    ...
    # keyword rule, so it stays uncategorized through the wizard - the
    # treasurer doesn't want any auto-generated "please fix this" hint
    # (e.g. a former "Uncategorized - add a rule" note) ever landing in
    # the Notes field, not even transiently during review, so an
    # uncategorized line's notes should just be blank end to end. Checked
    # here (rather than its own test) since this file shares one un-reset
    # DB across tests - only the first import of this fixture is
    # guaranteed not to be skipped as a duplicate.
    ...
    uncategorized = [e for e in imported_entries if not e["account_no"]]
    assert uncategorized, "expected an uncategorized line in the fixture"
    assert all(e["notes"] == "" for e in uncategorized)
