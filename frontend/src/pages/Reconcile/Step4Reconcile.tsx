...
/** Same "Reconcile" step as the Upload wizard's Step3Reconcile (same
 * merge-stripe call, same day-by-day breakdown table) - kept as its own
 * copy for the same reason ReconcileStepper.tsx is a copy (the deprecated
 * Upload wizard's files stay untouched), with one behavior change: by the
 * time a run reaches this page's flow, Date range/Review/Check Stripe
 * funds are already done, so there's nothing left to decide before
 * reconciling - it runs automatically on arrival instead of waiting for a
 * manual click. A "Re-run" button stays available for after edits. */
export default function Step4Reconcile(props: {
  run: ReconRun;
  ...
  const { widths, startResize } = useColumnWidths("reconcile-step4-by-day");
  const { widths: issueWidths, startResize: startIssueResize } = useColumnWidths(
    "reconcile-step4-issue-lines"
  );
  ...
  return (
    <div>
      <div className="card">
        ...
        {!done && !error && <p className="subtitle">{busy ? "Reconciling…" : ""}</p>}
        {done && (
          <button className="btn secondary" onClick={doReconcile} disabled={busy}>
            {busy ? "Re-running…" : "Re-run reconcile"}
          </button>
        )}
