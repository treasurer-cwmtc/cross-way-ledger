...
/** Same Stripe fund-coverage check the Upload wizard's Step2StripeUpload
 * shows (blocks progression until every donation fund has a rule) - kept
 * as its own copy for the same reason ReconcileStepper.tsx is a copy: the
 * deprecated Upload wizard's files stay untouched. See issue #122. */
export default function Step3FundCheck(props: {
  ...
  const { widths, startResize } = useColumnWidths("reconcile-step3-stripe-funds");
    ...
    <div>
      <div className="card">
        ...
        <p className="subtitle" style={{ marginTop: 0 }}>
          Fund coverage is checked against the Stripe transactions already synced. If that
          data looks out of date, go sync it on the previous step, then come back and
          re-check below.
        </p>
      ...
      {check && (
        <div className="card">
          ...
          {check.funds.length === 0 ? (
            <p className="subtitle">No donation funds found in the synced Stripe data yet.</p>
          ) : check.all_covered ? (
            <p className="ok">✓ All funds have a rule.</p>
          ) : (
            <p className="error">
              ✗ Some funds don't have a rule yet - add one below for each so donations land
