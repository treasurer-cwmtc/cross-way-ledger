...
| `campaign_donations` | `donations` | The Giving App's full donation export - not scoped to any one campaign; a campaign just claims a `fund` value. Synced live from the Giving API (`POST /api/pco/giving/donations/sync`), exploding a multi-fund donation into one row per fund; CSV upload kept as a fallback. |
...
People export, synced live from the People API (`POST /api/pco/people/sync`,
any status), CSV upload kept as a fallback.
